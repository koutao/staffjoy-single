package com.koutao.company.service;

import com.github.structlog4j.ILogger;
import com.github.structlog4j.SLoggerFactory;
import com.koutao.account.bean.AccountDto;
import com.koutao.account.bean.GenericAccountResponse;
import com.koutao.account.bean.GetOrCreateRequest;
import com.koutao.account.service.AccountService;
import com.koutao.account.service.helper.ServiceHelper;
import com.koutao.common.auth.AuthConstant;
import com.koutao.common.auth.AuthContext;
import com.koutao.common.bean.LogEntry;
import com.koutao.common.bean.ResultCode;
import com.koutao.common.error.ServiceException;
import com.koutao.company.bean.*;
import com.koutao.company.bean.entity.Directory;
import com.koutao.company.dao.CompanyRepo;
import com.koutao.company.dao.DirectoryRepo;
...
public class DirectoryService {
    ...
    public DirectoryEntryDto createDirectory(NewDirectoryEntry req) {
        ...
        try {
            genericAccountResponse = new GenericAccountResponse(accountService.getOrCreate(req.getName(), req.getEmail(), req.getPhoneNumber()));
        } catch (Exception ex) {
    ...
    public DirectoryList listDirectory(String companyId, int offset, int limit) {
        ...
        for(Directory directory : directoryPage.getContent()) {
            ...
            try {
                resp = new GenericAccountResponse(accountService.get(directory.getUserId()));
            } catch (Exception ex) {
    ...
    public DirectoryEntryDto getDirectoryEntry(String companyId, String userId) {
        ...
        GenericAccountResponse resp = null;
        try {
            resp = new GenericAccountResponse(accountService.get(userId));
        } catch (Exception ex) {
    ...
    public DirectoryEntryDto updateDirectoryEntry(DirectoryEntryDto request) {
        ...
        try {
            genericAccountResponse1 = new GenericAccountResponse(accountService.get(orig.getUserId()));
        } catch (Exception ex) {
        ...
        if (accountUpdateRequested) {
            ...
            try {
                genericAccountResponse2 = new GenericAccountResponse(accountService.update(account));
            } catch (Exception ex) {
